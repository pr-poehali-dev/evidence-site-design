...
const Navigation = ({ activeSection }: { activeSection: string }) => {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-12 py-8 flex justify-between items-center">
        <button
          onClick={() => scrollToSection('hero')}
          className="text-xs font-light tracking-[0.2em] uppercase text-primary hover-gold"
        >
          Екатерина Дэвиль
        </button>
        <div className="flex gap-16">
          {['approach', 'services', 'results', 'contact'].map((section) => (
            <button
              ...
              className={`text-xs font-light tracking-[0.15em] transition-all duration-500 hover-gold ${
    ...
    >
      ...
      <div className="container mx-auto px-12 relative z-10 text-center">
        <div className="space-y-6 max-w-5xl mx-auto">
          <h1
            className={`font-editorial text-8xl lg:text-[120px] font-normal tracking-tight leading-none text-primary text-glow transition-all duration-1000 ${
          ...
          <div className="h-px w-32 mx-auto bg-primary/30 my-12" />
          
          <p
            className={`text-sm tracking-[0.25em] uppercase text-primary/90 font-light transition-all duration-1000 delay-300 ${
        ...
        <p
          className={`absolute bottom-24 left-1/2 transform -translate-x-1/2 text-xs tracking-[0.2em] uppercase text-muted/60 font-light transition-all duration-1000 delay-700 ${
...
const ApproachSection = () => {
  ...
  return (
    <section id="approach" className="min-h-screen flex items-center py-32 bg-secondary/30">
      <div className="container mx-auto px-12">
        <div className="max-w-4xl mx-auto text-center space-y-12">
          {lines.map((line, index) => (
            <p
              key={index}
              className={`font-editorial text-4xl lg:text-5xl font-normal leading-relaxed text-foreground/90 animate-fade-in-slow ${
                line === '' ? 'my-8' : ''
              }`}
          ...
          <div className="pt-16">
            ...
            <p className="text-xs tracking-[0.2em] uppercase text-muted/70 font-light">
      ...
      <div className="container mx-auto px-12">
        <div className="max-w-3xl mx-auto space-y-20">
          {works.map((work, index) => (
            ...
            >
              <h3 className="font-editorial text-5xl font-normal mb-6 text-primary hover-gold">
                {work.title}
              </h3>
              <div className="w-16 h-px bg-primary/20 mb-6" />
              <p className="text-sm leading-loose text-muted/80 font-light lowercase tracking-wide">
                {work.text}
              </p>
            </div>
          ))}
          ...
          <div className="pt-12 border-t border-white/[0.08]">
            <p className="text-xs text-muted/50 font-light leading-relaxed">
...
const ServicesSection = () => {
  ...
  return (
    <section id="services" className="min-h-screen py-32 bg-secondary/20">
      <div className="container mx-auto px-12">
        <div className="max-w-4xl mx-auto">
          <div className="space-y-8 mb-24">
            {services.map((service, index) => (
              <div
                key={index}
                className="border-b border-white/[0.08] pb-8 cursor-pointer group animate-fade-in"
                style={{ animationDelay: `${index * 100}ms` }}
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
              >
                <div className="flex justify-between items-baseline mb-4">
                  <h3 className="font-editorial text-2xl font-normal text-foreground/90 hover-gold">
                    {service.title}
                  </h3>
                  <p className="text-lg font-light text-primary tracking-wide">
                    {service.price}
                  </p>
                </div>
                ...
                >
                  <p className="text-xs text-muted/70 font-light leading-relaxed pt-4">
                    {service.description}
                  </p>
                </div>
              </div>
...
const ResultsSection = () => {
  ...
  return (
    <section id="results" className="min-h-screen flex items-center py-32">
      <div className="container mx-auto px-12">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-20 mb-24">
            {results.map((result, index) => (
              <div
                ...
                style={{ animationDelay: `${index * 200}ms` }}
              >
                <h3 className="font-editorial text-5xl font-normal text-primary leading-tight">
                  {result.metric}
                </h3>
                <p className="text-sm text-muted/70 font-light leading-relaxed">
                  {result.context}
                </p>
                <p className="text-xs text-muted/50 font-light italic">
                  {result.detail}
                </p>
              </div>
            ))}
          ...
          <div className="space-y-8 pt-12 border-t border-white/[0.08]">
            {quotes.map((quote, index) => (
              <p
                key={index}
                className="font-editorial text-2xl font-normal text-muted/60 italic text-center animate-fade-in-slow"
...
const ContactSection = () => {
  return (
    <section id="contact" className="min-h-screen flex items-center py-32 bg-secondary/30">
      <div className="container mx-auto px-12">
        <div className="max-w-2xl mx-auto text-center space-y-16 animate-fade-in-slow">
          <div className="space-y-8">
            <a
              href="https://t.me/ekaterina_devile"
              className="block text-lg font-light tracking-[0.1em] text-foreground hover-gold"
            >
          ...
          <p className="text-xs tracking-[0.2em] uppercase text-muted/50 font-light">
